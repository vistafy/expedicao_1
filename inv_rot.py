...
def localizar_arquivo_inventario():
...
def salvar_no_banco(df, linhas_descartadas, lojas_nome_map):
    ...
    df[colunas_validas].to_sql("inventario_rotativo", conn, if_exists="replace", index=False)
    conn.commit()

    resumo_lojas = df.groupby("loja_id")["valor_total"].sum().reset_index()
    conn.close()

    print("💾 Dados inseridos na tabela inventario_rotativo (SQLite).")
...
if __name__ == "__main__":
    arquivo = localizar_arquivo_inventario()
    if arquivo:
